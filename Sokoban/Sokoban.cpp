...
void GameObject::setBlock(int _x, int _y, typeObject _object,
    ...
    if (_object == playerDown || _object == playerLeft
        || _object == playerRight || _object == playerUp) {
            object = player;
    }
}
...
// originaly there
bool Sokoban::executeMoveObjects(moveSet move) {
    GameObject replaceBlock, movingBlock;
    typeObject playerMovement = player;
    ...
    if (nextPlayerMove.getMovable() == false
        ...
        replaceBlock.setBlock(oldPosition.first, oldPosition.second,
                    getTypeUnderAtPosition(oldPosition), textureMap);
        replaceBlock.setCurrTypeUnder(replaceBlock.getTypeObject());
        // moving block is player
        playerMovement = getPlayerMovement(move);
        movingBlock.setBlock(newPosition.first,
                    newPosition.second, playerMovement, textureMap);
...
typeObject Sokoban::getPlayerMovement(moveSet move) {
    switch (move) {
    case up:
        return playerUp;
    case down:
...
bool Sokoban::buildGameObjectVector(ifstream& in, vector<GameObject>& objects) {
    ...
    while ( !(in.eof()) ) {
        ...
        switch ( input ) {
        case '@':
            texture.loadFromFile("sokoban/images/player_up.png");
            setTextureInMap(playerUp, texture);
            texture.loadFromFile("sokoban/images/player_down.png");
            setTextureInMap(playerDown, texture);
            texture.loadFromFile("sokoban/images/player_left.png");
            setTextureInMap(playerLeft, texture);
            texture.loadFromFile("sokoban/images/player_right.png");
            setTextureInMap(playerRight, texture);
