...
enum typeObject{nill, player, playerDown, playerUp,
playerLeft, playerRight, wall, floor, crate, storage_space, stored};
...
class GameObject {
 ...
 public:
    ...
    // mutators
    void setBlock(int _x, int _y, typeObject _object,
...
class Sokoban : public sf::Drawable {
 public:
    ...
    void setPlayerPosition(int x, int y) {playerLocation.first = x,
 ...
 private:
    ...
    bool executeMoveObjects(moveSet move);
    // pre: is given two valid move blocks
    // post: places them in matrix location
    void moveMatrixBlocks(const GameObject& replaceBlock,
                              const GameObject& movingBlock);
    // pre: is given a move
    // post: returns enum for animation type
    typeObject getPlayerMovement(moveSet move);

    int xSize;
    int ySize;
    int pointsToWin;
    typeObject lastMove = player;
...
void setDrawPos(sf::Vector2f& location, const GameObject& block,
                  const pair<int, int>& xyCor);
